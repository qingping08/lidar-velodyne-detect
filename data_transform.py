...
##print(coordinate_transform.transform_2_1(1,1,1))

""" r_2=torch.tensor(        [[ 2.0990e+01,  5.3159e+00, -1.9987e+00],
         [ 2.1011e+01,  6.9806e+00, -1.9987e+00],
         [ 2.4840e+01,  6.9326e+00, -1.9987e+00],
         [ 2.4819e+01,  5.2679e+00, -1.9987e+00],
         [ 2.0990e+01,  5.3159e+00, -5.6340e-01],
         [ 2.1011e+01,  6.9806e+00, -5.6340e-01],
         [ 2.4840e+01,  6.9326e+00, -5.6340e-01],
         [ 2.4819e+01,  5.2679e+00, -5.6340e-01]])  """
""" r_2=np.array(        [[ 2.0990e+01,  5.3159e+00, -1.9987e+00],
         [ 2.1011e+01,  6.9806e+00, -1.9987e+00],
         [ 2.4840e+01,  6.9326e+00, -1.9987e+00],
         [ 2.4819e+01,  5.2679e+00, -1.9987e+00],
         [ 2.0990e+01,  5.3159e+00, -5.6340e-01],
         ...
         [ 2.4840e+01,  6.9326e+00, -5.6340e-01],
         [ 2.4819e+01,  5.2679e+00, -5.6340e-01]]) """
#f=open('data_transform','w')                              
""" for i in range(0,8):
    x=float(r_2[i][0])
    y=float(r_2[i][1])
    z=float(r_2[i][2])
    tmp=coordinate_transform.transform_2_1(x,y,z)
    print(tmp) """
"""     f.write(str(tmp[0]))
    f.write(' ')
    f.write(str(tmp[1]))
...
def coor_tran(box):
    ...
    for i in range(0,8):
        ...
        tmp=coordinate_transform.transform_2_1(x,y,z)
        if i==0:
            coordinate=np.array([float(tmp[0]),float(tmp[1]),float(tmp[2])])
        else:
            tmp_r=np.array([float(tmp[0]),float(tmp[1]),float(tmp[2])])
            coordinate=np.row_stack((coordinate,tmp_r))
    return torch.from_numpy(coordinate)
def get_converted_tensor(ref_3d):
    ref_3d_converted= torch.Tensor(len(ref_3d),8, 3)
    for i in range(0,len(ref_3d)):
           box_2=ref_corners3d2[i]
           box_2_converted=coor_tran(box_2)
           ref_3d_converted[i]=box_2_converted
    return ref_3d_converted
#print(get_converted_tensor(ref_corners3d2))
